...
class CustomTimerWidget extends StatefulWidget {
  ...
  const CustomTimerWidget({super.key, required this.builder, this.autoStart = false, required this.controller, this.onFinish});
  @override
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<CustomTimer>(
      ...
      builder: (context, snapshot) {
        return widget.builder(snapshot.data ?? widget.controller._getInitData());
      },
    );
  }
}
...
class CustomTimer {
  ...
  CustomTimer({this.days = 0, this.hours = 0, this.minutes = 0, this.seconds = 0});
  @override
  toString() {
...
class CustomTimerController {
  ...
  final StreamController<CustomTimer> _controller = StreamController.broadcast();
  Timer? timer;
  late int _sec;
  _startCounterDown() {
